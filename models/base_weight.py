...
# Base models for different task using BERT pretrained model.
...
class MultiLabelClassificationBase(nn.Module):
    ...
    def forward(self, pooled_output, labels=None):
        ...
        if labels is not None:
            loss_fct = nn.BCEWithLogitsLoss(reduction='none', pos_weight=self.label_weight)
            loss = loss_fct(logits, labels)

            #print(logits.tolist())
            #print(torch.masked_select(torch.sigmoid(logits), labels.ge(1)))
            #print(labels.tolist())
            pos_loss = torch.masked_select(loss, labels.ge(1))
            print("pos loss: %s" % pos_loss.tolist())
            neg_loss = torch.masked_select(loss, labels.le(0))
            print("neg loss: %s" % neg_loss.tolist())
            return loss.mean(dim=-1), pos_loss.mean(dim=-1), neg_loss.mean(dim=-1), logits 
            #return pos_loss.mean(dim=-1), pos_loss.mean(dim=-1), neg_loss.mean(dim=-1), logits 
        else:
            return torch.sigmoid(logits) #logits = torch.clamp(torch.sigmoid(self.classifier(pooled_output)), 1e-8, 1.0)
        #logits = self.classifier(pooled_output)
        #if labels is not None:
    ...
    #def forward(self, pooled_output, labels=None):
    #    pooled_output = self.dropout(pooled_output)
    #    logits = torch.clamp(torch.sigmoid(self.classifier(pooled_output)), 1e-8, 1.0)
    #    #logits = self.classifier(pooled_output)
    #    if labels is not None:
    #        #loss_fct = nn.BCELoss(reduction='none')
    #        #loss = loss_fct(logits, labels)
    #        zeros = torch.zeros(labels.shape).to('cuda')
    #        pos_p_sub = torch.where(labels > zeros, labels - logits, zeros)
    #        neg_p_sub = torch.where(labels > zeros, zeros, logits)
    #        alpha = 0.25
    #        gamma = 2.0
    #        F_loss = - (1 - alpha) * (pos_p_sub ** gamma) * torch.log(torch.clamp(logits, 1e-8, 1.0)) - alpha * (
    #                        neg_p_sub ** gamma) * torch.log(torch.clamp(1.0 - logits, 1e-8, 1.0))
    #        pos_loss = torch.masked_select(F_loss, labels.ge(1))
    #        neg_loss = torch.masked_select(F_loss, labels.le(0))
    #        return F_loss.mean(dim=-1), pos_loss.mean(dim=-1), neg_loss.mean(dim=-1), logits
    #    return logits
